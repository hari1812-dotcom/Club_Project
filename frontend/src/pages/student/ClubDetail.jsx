      ...
      <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-5 flex flex-col gap-4 max-h-[70vh] overflow-y-auto">
          ...
          <div className="flex gap-3 pt-1">
            ...
            <button type="submit" disabled={submitting} className={`flex-[2] py-3 text-white font-bold text-sm rounded-xl transition shadow-sm disabled:opacity-60 bg-gradient-to-r ${c.bannerFrom} ${c.bannerTo}`}>
  ...
  return (
    <DashboardLayout title={club.name}>
      ...
      <div className="student-root" style={{ maxWidth: 780, margin: "0 auto" }}>
        ...
        <button
          onClick={() => navigate(-1)}
          className={`flex items-center gap-2 ${c.bg} border border-${c.text.replace("text-","")} ${c.text} text-sm font-semibold px-4 py-2 rounded-xl mb-6 hover:opacity-80 transition`}
        >
        ...
        <div className={`rounded-2xl bg-gradient-to-r ${c.bannerFrom} ${c.bannerTo} px-7 py-6 mb-5 flex items-start justify-between gap-4 flex-wrap`}>
          ...
          <div className="bg-white/20 border border-white/30 rounded-2xl px-5 py-4 text-center shrink-0">
            ...
            {club.memberLimit && <p className="text-white/60 text-xs mt-1">/ {club.memberLimit} max</p>}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
          ...
          <div className={`detail-card ${c.bg} rounded-2xl p-5`}>
            ...
            ) : <p className="text-slate-400 text-sm">No activities listed yet.</p>}
          </div>
        </div>
        ...
        <div className="detail-card bg-white rounded-2xl border border-slate-100 shadow-sm p-5 mb-4">
          ...
          {faculty ? (
            <div className="flex items-start gap-4">
              <div className={`w-12 h-12 ${c.iconBg} rounded-xl flex items-center justify-center ${c.text} font-bold text-lg shrink-0`}>
                {faculty.name?.[0]?.toUpperCase() || "F"}
              </div>
              <div className="flex flex-col gap-1.5">
                <p className="font-bold text-slate-800 text-sm">{faculty.name}</p>
                {faculty.department && <p className="text-slate-400 text-xs">{faculty.department}</p>}
                {faculty.email && <p className="text-slate-400 text-xs">{faculty.email}</p>}
                {faculty.phone && <p className="text-slate-400 text-xs">{faculty.phone}</p>}
              </div>
            </div>
          ) : <p className="text-slate-400 text-sm">Incharge details not available.</p>}
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
          {isMember || requestStatus === "Approved" ? (
            ...
            <div className="flex items-center justify-between flex-wrap gap-4">
              ...
              <button
                className={`join-btn flex items-center gap-2 px-6 py-3 text-white font-bold text-sm rounded-xl shadow-sm bg-gradient-to-r ${c.bannerFrom} ${c.bannerTo}`}
