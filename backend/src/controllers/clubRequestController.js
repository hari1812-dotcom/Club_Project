...
// Student: request to join a club via Google Form link
exports.requestToJoin = async (req, res, next) => {
  try {
    const { clubId, formResponseUrl, reason } = req.body;

    if (!clubId || !formResponseUrl) {
      return res
        .status(400)
        .json({ message: "clubId and formResponseUrl are required" });
    }
    ...
    if (existingReq && existingReq.status !== "Rejected") {
      return res
        .status(400)
        .json({ message: "You already have a request for this club" });
    }
    ...
    const request =
      existingReq && existingReq.status === "Rejected"
        ? await ClubRequest.findByIdAndUpdate(
            existingReq._id,
            { status: "Pending", formResponseUrl, reason },
            { new: true }
          )
    ...
    // Notify faculty incharge
    if (club.facultyIncharge) {
      await Notification.create({
        userId: club.facultyIncharge._id,
        message: `New join request for ${club.name}`,
      });
    }
...
exports.updateRequestStatus = async (req, res, next) => {
  try {
    ...
    // Ensure faculty manages this club
    if (club.facultyIncharge.toString() !== req.user.id) {
    ...
    if (status === "Approved") {
      ...
      if (currentMembers >= club.maxCapacity) {
        return res
          .status(400)
          .json({ message: "Member capacity reached for this club" });
      }
